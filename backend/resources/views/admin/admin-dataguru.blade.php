...
<body class="bg-[#f4f7f6] font-sans flex text-gray-800">
    ...
    <aside class="w-72 min-h-screen bg-[#10b981] text-white flex flex-col fixed shadow-xl z-50">
        ...
        <nav class="mt-4 flex-grow pl-6">
            ...
            <!-- Link Data Guru menggunakan sistem View -->
    ...
    <main class="flex-1 ml-72 p-10 relative">
        ...
        <!-- ALERTS (Menampilkan Pesan Sukses/Error) -->
        @if (session('success'))
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-[#005c4b] text-white text-[10px] font-black uppercase tracking-widest">
                        <tr>
                            ...
                            <th class="py-4 text-center">Wali Kelas</th>
                    ...
                    <tbody class="text-xs divide-y divide-gray-100">
                        @forelse($dataGuru ?? [] as $guru)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="py-5 text-gray-500">{{ $guru->pendidikan ?? '-' }}</td>
        ...
        <div class="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl">
            ...
            <form action="{{ route('admin.guru.store') }}" method="POST" class="p-6">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-600 mb-1">Wali Kelas</label>
                        <select name="wali_kelas"
                            class="w-full border rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#10b981]">
                            <option value="">-- Bukan Wali Kelas --</option>
                            @foreach (['VII', 'VIII', 'IX'] as $tingkat)
                                <optgroup label="Kelas {{ $tingkat }}">
                                    @foreach (range('A', 'K') as $kelas)
                                        <option value="{{ $tingkat }}.{{ $kelas }}">
                                            {{ $tingkat }}.{{ $kelas }}</option>
                                    @endforeach
                                </optgroup>
                            @endforeach
                        </select>
    ...
    <div id="editModal" class="fixed inset-0 bg-black/50 hidden z-[60] flex items-center justify-center">
        <div class="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl">
            ...
            <!-- Action form akan diubah lewat Javascript -->
    ...
    <script>
        ...
        function showView(viewId) {
            ...
            if (viewId === 'data-guru') {
                ...
                // Tidak lagi mencari nav-profile karena sudah dihapus dari sidebar
        ...
        function openEditModal(guru) {
            ...
            // Set action URL form menggunakan helper Laravel yang aman untuk Laragon
